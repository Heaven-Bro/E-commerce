...
def update_cart_item(request):
    ...
    try:
        ...
        if int(quantity) < 1:
            item.delete()
            return Response({'error': 'Quantity must be at least 1. Item removed from cart.'}, status=status.HTTP_400_BAD_REQUEST)
...
def register_view(request):
    ...
    if serializer.is_valid():
        ...
        return Response({"message":"User created sucessfully", "user" : UserSerializer(user).data}, status=status.HTTP_201_CREATED)
